/* eslint-disable quotes */
/* eslint-disable keyword-spacing */
/* eslint-disable space-before-blocks */
/* eslint-disable brace-style */
/* eslint-disable space-in-parens */
/* eslint-disable no-trailing-spaces */
/* eslint-disable indent */
/* eslint-disable block-spacing */
/* eslint-disable key-spacing */
import _ from 'lodash';
import axios from 'axios';
let config = require('../../config');

let backendConfigurer = function () {
  switch (process.env.NODE_ENV) {
    case 'testing':
    case 'development':
      return 'http://' + config.dev.backendHost + ':' + config.dev.backendPort;
    case 'production':
      return 'https://' + config.build.backendHost + ':' + config.build.backendPort;
  }
}

let backendUrl = backendConfigurer();

let AXIOS = axios.create({
  baseURL: backendUrl
  // headers: {'Access-Control-Allow-Origin': frontendUrl}
});

export default {
  name: 'eventregistration',
  data () {
    return {
      persons: [],
      promoters: [],
      events: [],
      newPerson: '',
      personType: '',
      newEvent: {
        name: '',
        date: '10-10-2010',
        startTime: '09:00',
        endTime: '11:00'
      },
      selectedPerson: '',
      selectedPromoter: '',
      selectedEvent: '',
      selectedEventP: '',
      selectedPersonC: '',
      selectedEventC: '',
      deviceId: '',
      amount: '',
      errorPerson: '',
      errorEvent: '',
      errorPay:'',
      errorAssign:'',
      errorRegistration: '',
      response: []
    }
  },
  created: function () {
    // Initializing persons
    AXIOS.get('/persons')
    .then(response => {
      this.persons = response.data;
      this.persons.forEach(person => this.getRegistrations(person.name))
    })
    .catch(e => {this.errorPerson = e});

    // initialize promoters
    AXIOS.get('/promoters')
    .then(response => {
      this.promoters = response.data;
      this.promoters.forEach(promoter => {
        this.getRegistrations(promoter.name); 
        // this.persons.push(promoter)
      })
    })
    .catch(e => {this.errorPerson = e});
    AXIOS.get('/events').then(response => {this.events = response.data; this.events.forEach(event => {
        if (!event.make ) {
          event.make = '--';
        }    
      }
      )}).catch(e => {this.errorEvent = e });
  },

  methods: {

    createPerson: function (personType, personName) {
      if(personType === "promoter"){
        AXIOS.post('/promoters/'.concat(personName), {}, {})
        .then(response => {
          this.persons.push(response.data);
          this.promoters.push(response.data);
          this.errorPerson = '';
          this.newPerson = '';
        })
        .catch(e => {
          e = e.response.data.message ? e.response.data.message : e;
          this.errorPerson = e;
          console.log(e);
        });
      }else{
        AXIOS.post('/persons/'.concat(personName), {}, {})
        .then(response => {
          this.persons.push(response.data);
          this.errorPerson = '';
          this.newPerson = '';
        })
        .catch(e => {
          e = e.response.data.message ? e.response.data.message : e;
          this.errorPerson = e;
          console.log(e);
        });
      }
    },

    createEvent: function (newEvent) {
      let url = '';
      if(!newEvent.make){
        AXIOS.post('/events/'.concat(newEvent.name), {}, {params: newEvent})
        .then(response => {
          var event = response.data;
          event.make = '--';
          this.events.push(event);
          this.errorEvent = '';
          this.newEvent.name = this.newEvent.make = this.newEvent.movie = this.newEvent.company = this.newEvent.artist = this.newEvent.title = '';
        })
        .catch(e => {
          e = e.response.data.message ? e.response.data.message : e;
          this.errorEvent = e;
          console.log(e);
        });
      }else {
        AXIOS.post('/carshows/'.concat(newEvent.name), {}, {params: newEvent})
        .then(response => {
          this.events.push(response.data);
          this.errorEvent = '';
          this.newEvent.name = this.newEvent.make = this.newEvent.movie = this.newEvent.company = this.newEvent.artist = this.newEvent.title = '';
        })
        .catch(e => {
          e = e.response.data.message ? e.response.data.message : e;
          this.errorEvent = e;
          console.log(e);
        });
      }
    },

    registerEvent: function (personName, eventName) {
      let event = this.events.find(x => x.name === eventName);
      let person = this.persons.find(x => x.name === personName);
      let params = {
        person: person.name,
        event: event.name
      };

      AXIOS.post('/register', {}, {params: params})
      .then(response => {
        person.eventsAttended.push(event)
        this.selectedPerson = '';
        this.selectedEvent = '';
        this.errorRegistration = '';
      })
      .catch(e => {
        e = e.response.data.message ? e.response.data.message : e;
        this.errorRegistration = e;
        console.log(e);
      });
    },

    assignEvent: function (personName, eventName) {
      let event = this.events.find(x => x.name === eventName);
      let promoter = this.promoters.find(x => x.name === personName);
      let params = {
        promoter: promoter.name,
        event: event.name
      };

      AXIOS.post('/assign', {}, {params: params})
      .then(response => {
        // person.eventsAttended.push(event)
        this.selectedPerson = '';
        this.selectedEventP = '';
        this.errorAssign = '';
        console.log('assigned')
      })
      .catch(e => {
        e = e.response.data.message ? e.response.data.message : e;
        this.errorAssign = e;
        console.log(e);
      });
    },

    getRegistrations: function (personName) {
      // AXIOS.get('/events/person/'.concat(personName))
      AXIOS.get('/registrations/person/'.concat(personName))
      .then(response => {
        if (!response.data || response.data.length <= 0) return;

        let indexPart = this.persons.map(x => x.name).indexOf(personName);
        this.persons[indexPart].eventsAttended = [];
        // response.data.forEach(event => {
        //   this.persons[indexPart].eventsAttended.push(event);
        // });
        response.data.forEach(registration => {
          if(registration.creditCard !== null){
            registration.event.amount = registration.creditCard.amount;
            registration.event.deviceId = registration.creditCard.accountNumber;
          }
          this.persons[indexPart].eventsAttended.push(registration.event);
        });
      })
      .catch(e => {
        e = e.response.data.message ? e.response.data.message : e;
        console.log(e);
      });
    },

    pay: function (personName, eventName, deviceId, amount){
      let event = this.events.find(x => x.name === eventName);
      let person = this.persons.find(x => x.name === personName);
      let params = {
        person: person.name,
        event: event.name,
        deviceId: deviceId,
        amount: amount
      };

      AXIOS.post('/pay', {}, {params: params})
      .then(response => {
        console.log("success pay");
        
        let eventAttended = person.eventsAttended.find(x => x.name === eventName);
        eventAttended.amount = amount;
        eventAttended.deviceId = deviceId;
        this.selectedPersonC = '';
        this.selectedEventC = '';
        this.deviceId = '';
        this.amount = '';
        this.errorPay = '';
      })
      .catch(e => {
        console.log("error pay");
        e = e.response.data.message ? e.response.data.message : e;
        this.errorPay = e;
        console.log(e);
      });
    }
  }
}
